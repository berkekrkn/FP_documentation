...
%     [K,E] = ellipke(k^2);
%     M = -mu_0*sqrt(r1*r2)*((k-2/k)*K+2/k*E);
% end
    ...
    for j = 1:N_V % (i,j) of chosen winding
        for k = 1:N_L
            for l = 1:N_V % (k,l) of compared winding
                if i ~= k || j ~= l
                    m_ijkl = mutual_ind_corrected(r(i), r(k), z(j), z(l), d_i);
...
% figure;
% zz = linspace(0.001,1,1001);
% M1 = zeros(1, length(zz)); M2 = zeros(1, length(zz));
% for i = 1:length(zz)
%     M1(i) = mutual_ind(0.795e-3, 0.795e-3, zz(i)*1e-3);
%     M2(i) = mutual_ind(0.98e-3, 0.795e-3, zz(i)*1e-3);
% end
% plot(zz, M1*1e9, 'LineWidth', 1.5);
% xlim([0,1]);
% 
% plot(zz, M1*1e9, 'LineWidth', 1.5);
% xlim([0,1]);
